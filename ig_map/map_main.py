...
def extract_map_url(text):
    ...
    # 擴充 Regex 規則，包含常見的 Google Maps 格式
    match = re.search(r'(https?://(?:maps\.app\.goo\.gl|goo\.gl/maps|www\.google\.com/maps|google\.com/maps)[^\s]+)', text)
...
def determine_category(title):
    ...
    food_keywords = ["餐廳", "咖啡", "Coffee", "Cafe", "麵", "飯", "食", "味", "餐酒館", "Bar", "甜點", "火鍋", "料理", "Bistro", "早午餐"]
    travel_keywords = ["車站", "公園", "山", "海", "寺", "廟", "博物館", "步道", "農場", "樂園", "展覽", "View", "Hotel", "民宿", "景點"]
...
# --- 3. 核心功能 A: 存檔模式 (Debug Mode) ---

def handle_save_task(raw_message, user_id, reply_token):
    print(f"📥 [存檔模式] 開始處理...")
    # [Debug] 這是最關鍵的一行，讓我們看到 Make 到底傳了什麼過來
    print(f"🕵️ [DEBUG] 收到原始字串 -> [{raw_message}]")

    if not raw_message or not raw_message.strip():
        print("⚠️ [DEBUG] 訊息為空，結束程序")
        return

    target_url = extract_map_url(raw_message)
    print(f"🕵️ [DEBUG] Regex 抓到的網址 -> [{target_url}]")
    ...
    if target_url:
        ...
        if lat and lng:
            data = {
                "user_id": user_id,
                "location_name": temp_title, # 對應 map_spots
                "google_map_url": final_url, # 對應 map_spots
                "address": final_url,
                "latitude": lat,
                "longitude": lng,
                "category": category,
                "geom": f"POINT({lng} {lat})", # PostGIS
                "created_at": "now()"
            }
            try:
                ...
                print(f"❌ 資料庫寫入失敗: {e}")
                message_to_user = "❌ 系統錯誤，儲存失敗 (可能是資料庫欄位問題)。"
        else:
            print("⚠️ [DEBUG] 有網址但抓不到座標")
            backup_save(user_id, temp_title, raw_message, target_url)
            message_to_user = "⚠️ 連結已接收，但無法解析座標 (已存入待處理)。"
    else:
        print("⚠️ [DEBUG] Regex 沒有抓到任何網址，視為純文字筆記")
        backup_save(user_id, temp_title, raw_message, "")
        message_to_user = "📝 已將此訊息存為純文字筆記。"
